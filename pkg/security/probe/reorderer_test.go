...
func TestOrderRetention(t *testing.T) {
	...
	var count int
	heap.dequeue(func(record *perf.Record) { count++ }, 1, &metric, &metric, &ReOrdererOpts{})
	assert.Equal(t, 30, count)
	heap.dequeue(func(record *perf.Record) { count++ }, 2, &metric, &metric, &ReOrdererOpts{})
	assert.Equal(t, 60, count)
	heap.dequeue(func(record *perf.Record) { count++ }, 3, &metric, &metric, &ReOrdererOpts{})
